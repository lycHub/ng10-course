...
### @Host()
这个修饰符和@Self很像，但它的范围比@Self大一些，只要是宿主上提供的服务都能找到，比如
```angular2html
<ul>
  <app-mobile-item appCar *ngFor="let item of mobiles;"></app-mobile-item>
</ul>
```
appCar指令用在了MobileItemComponent上，即使MobileItemComponent组件中没有提供CarService，
<br />
只要在appCar指令中提供了，依然可以找到
```typescript
import { Directive } from '@angular/core';
import {CarServices} from './tokens';

@Directive({
  selector: '[appCar]',
  providers: [
    { provide: CarServices, useValue: { brand: 'jeep' } }
  ]
})
export class CarDirective {
  constructor() {
    console.log('car directive');
  }
}
```
